...
树的兄弟表示法为`实现树、森林与二叉树`之间的转换提供了基础，充分利用二叉树的有关算法来实现树及森林的操作，对难以把握的树和森林有着重要的意义



![孩子的兄弟表示法](./../../sources/images/tree-child-brother-descr.png)



树结构中，位于同一层的节点之间互为兄弟节点。例如，图 1 的普通树中，节点 A、B 和 C 互为兄弟节点，而节点  D、E 和 F 也互为兄弟节点。

<font color=red>孩子兄弟表示法，采用的是链式存储结构，其存储树的实现思想是：从树的根节点开始，依次用链表存储各个节点的孩子节点和兄弟节点。</font>
